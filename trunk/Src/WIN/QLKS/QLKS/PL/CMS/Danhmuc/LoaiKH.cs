...
{
    public partial class LoaiKH : UserControl
    {
        ...
        {
            this.ucLink1.SetValues("Địa chỉ", "Loại khách hàng");
        }
        private void Event()
        {
        ...
        void toolbar1_Delete()
        {
            ...
            AddCheckbox(LT_LOAIKHBUS.SelectAll());
        ...
        LT_LOAIKH AddDuLieu(LT_LOAIKH lkhs)
        {
            try
            {
                lkhs.MALKH = txtlkh.Text;
                lkhs.TENLKH = txttenlkh.Text;
            }
            catch { }
            return lkhs;
        }
        void Save()
        {
            ...
            if (flag == 1)
            {
                if (LT_LOAIKHBUS.Insert(AddDuLieu(lkh)) == true)
                {
                    f = new frmHoidap("Thêm Loại khách hàng thành công!", frmHoidap.CO.an, frmHoidap.KHONG.an, frmHoidap.THOAT.an, frmHoidap.HINH.thongtin);
                    f.ShowDialog();
                    LoadData();
                    this.toolbar1.SetSave();
                    Clear();
                ...
                {
                    f = new frmHoidap("Thêm Loại khách hàng  có lỗi xảy ra!", frmHoidap.CO.an, frmHoidap.KHONG.an, frmHoidap.THOAT.an, frmHoidap.HINH.baoloi);
                    f.ShowDialog();
                    LoadData();
                    this.toolbar1.SetSave();
                    Clear();
            ...
            if (flag == 2)
            {
                try
                {
                    f = new frmHoidap("Bạn có muốn sửa Loại khách hàng  ?", frmHoidap.CO.hien, frmHoidap.KHONG.hien, frmHoidap.THOAT.an, frmHoidap.HINH.canhbao);
                    if (f.ShowDialog() == DialogResult.Yes)
                    {
                        if (LT_LOAIKHBUS.Update(AddDuLieu(lkh)) == true)
                        {
                            f = new frmHoidap("Sửa Loại khách hàng  thành công!", frmHoidap.CO.an, frmHoidap.KHONG.an, frmHoidap.THOAT.an, frmHoidap.HINH.thongtin);
                            f.ShowDialog();
                            LoadData();
                            this.dgvgd.ReadOnly = true;
                            this.toolbar1.SetSave();
                        ...
                        {
                            f = new frmHoidap("Có lỗi khi Sửa Loại khách hàng  !", frmHoidap.CO.an, frmHoidap.KHONG.an, frmHoidap.THOAT.an, frmHoidap.HINH.baoloi);
                            f.ShowDialog();
                            this.toolbar1.SetSave();
                            this.dgvgd.ReadOnly = true;
                            Clear();
            ...
            if (flag == 3)
            {
                f = new frmHoidap("Bạn có chắc chắn muốn xóa dữ liệu Loại khách hàng  vừa chọn?", frmHoidap.CO.hien, frmHoidap.KHONG.hien, frmHoidap.THOAT.an, frmHoidap.HINH.dauhoi);
                if (f.ShowDialog() == DialogResult.Yes)
                {
                    foreach (DataGridViewRow dr in this.dgvgd.Rows)
                    {
                        try
                        {
                            ...
                            {
                                lkh = new LT_LOAIKH();
                                lkh.MALKH = dr.Cells["MALKH"].Value.ToString();
                                LT_LOAIKHBUS.Delete(lkh);
                            }
                        }
                        catch
                        {
                            f = new frmHoidap("Bạn chưa chọn phần Loại khách hàng nào vui lòng chọn lại!", frmHoidap.CO.an, frmHoidap.KHONG.an, frmHoidap.THOAT.an, frmHoidap.HINH.thongtin);
                            f.ShowDialog();
                        }
                    }
                    f = new frmHoidap("Xóa thành công Loại khách hàng !", frmHoidap.CO.an, frmHoidap.KHONG.an, frmHoidap.THOAT.an, frmHoidap.HINH.thongtin);
                    f.ShowDialog();
                    RemoveCheckbox();
                    this.toolbar1.SetSave();
                    this.dgvgd.ReadOnly = true;
        ...
        {
            lkh = new LT_LOAIKH();
            this.txtlkh.Text = LT_LOAIKHBUS.GetID();
        }
        private void Clear()
        {
        ...
        void LoadData()
        {
            Common.HightLightDataGridViewRow(ref this.dgvgd, LT_LOAIKHBUS.SelectAll());
            this.dgvgd.Columns["MALKH"].Width = 100;
        }
        ...
        void ChooseData(object sender, int e)
        {
            if (flag == 2)
            {
                try
                {
                    ...
                    txtlkh.Text = dr.Cells["MALKH"].Value.ToString();
                    txttenlkh.Text = dr.Cells["TENLKH"].Value.ToString();
                }
                catch { return; };
            }
        }
        ...
        private void LoaiKH_LocationChanged(object sender, EventArgs e)
        {
            Event();
            SetLink();
        }
